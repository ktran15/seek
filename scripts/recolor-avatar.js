...
function floodMask(png, seedBand, growBand) {
  ...
  while (stack.length) {
    ...
    for (const q of [p - 1, p + 1, p - w, p + w]) {
      ...
      if (data[q * 4 + 3] < 64) continue;
      const hsl = rgbToHsl(data[q * 4], data[q * 4 + 1], data[q * 4 + 2]);
      if (inBand(hsl, growBand)) { mask[q] = 1; stack.push(q); }
    }
  }
  return mask;
}
...
function recolor(srcPath, outPath, band, targetHex, baseL, opts = {}) {
  ...
  const { exclude, yRange = [0, png.height] } = opts;
...
if (mode === 'skin') {
  ...
  const opts = { exclude: shirtMask, yRange: [96, 854] };
