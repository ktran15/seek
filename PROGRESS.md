...
| 9 | Batch generation: eyes (3), hair (5×5), cosmetics (32), mascot states (cheer/defeat), badges (4), flags/trail/summit, logo/loading | ⬜ **BLOCKED on key** (see 10) |
| 10 | Throttled Gemini pipeline: `scripts/art-manifest.js` (reference-conditioned per Rig Bible §6, one item/call, worn-on-base recipe for registration) + `scripts/generate-art.js` (HARD caps: 10 calls/burst + 60s wait, 250/day persisted in gitignored `.gen-quota.json`, resume skips finished outputs, raw results → gitignored `assets/art/inbox/`). Shakedown run (bald base + mascot cheer/defeat + 4 hats = 7 calls) executed 2026-07-09: throttle/retry/quota verified live, but **every call 429'd with `free_tier_requests, limit: 0` — the Gemini FREE tier has no allowance for the image model at all.** Founder must either enable billing on the key's Google Cloud project (~$0.04/image ≈ $3 for the whole batch), swap in a key that has image quota (collaborator's), or fall back to the manual generate-in-app workflow used for the anchors | 🔶 pipeline done — **generation blocked on founder key decision** |
