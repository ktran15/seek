...
### Security audit remediation (2026-07-11) — founder-directed "fix everything"
Full audit ran first (no Criticals; RLS on all tables, server-authoritative writes, no client service key). Fixes applied — migration `20260711000001_m13_security_hardening.sql` (**⬜ FOUNDER ACTION: `npx supabase db push`**; read its header — the M1 unique-index build fails if past races left duplicate ledger rows, verify with the query there before applying):
...
- **HELD for founder review — M3 (profiles RLS):** the `profiles` SELECT policy is `using(true)`, so a blocked user can still read another's base row and `coins` is visible to any authenticated user. The fix touches a SELECT policy on a table read everywhere (feeds, friends, author display) — per the standing "RLS changes need a second confirm" rule, NOT applied; diff pending founder sign-off.
